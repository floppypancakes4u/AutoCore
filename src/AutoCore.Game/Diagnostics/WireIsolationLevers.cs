...
public static class WireIsolationLevers
{
    ...
    private static readonly LeverDef[] LeverDefs =
    {
        ...
        new("EnableCreatureMovingPriority", "Moving creatures compete for pose slots near vehicle level (off = legacy 0.15 residual)",
            () => GhostCreature.EnableCreatureMovingPriority, v => GhostCreature.EnableCreatureMovingPriority = v, envSuffix: "CREATURE_POSE_PRIORITY"),
        new("CreatureMotionDiag", "Count direction reversals in the transmitted creature pose stream (server-motion vs transport)",
            () => CreatureMotionDiag.Enabled, v => CreatureMotionDiag.Enabled = v, envSuffix: "CREATURE_MOTION_DIAG"),
        new("PublishSteerGoal", "Send a real move-to goal so the CLIENT AI walks creatures (off = goal==position, server-authoritative)",
            () => SoftNpcPathMotion.PublishSteerGoalLever, v => SoftNpcPathMotion.PublishSteerGoalLever = v,
            envSuffix: "PUBLISH_STEER_GOAL"),
        new("EnableNpcDepartureStagger", "Spread NPCs sharing a MapPath by offsetting first departure (off = legacy lockstep clumps)",
            () => SoftNpcPathMotion.MaxStaggerDelayMs > 0,
            v => SoftNpcPathMotion.MaxStaggerDelayMs = v ? 6000 : 0,
            envSuffix: "NPC_DEPARTURE_STAGGER"),
