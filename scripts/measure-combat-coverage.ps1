...
if ([string]::IsNullOrWhiteSpace($CoverageFile) -or -not (Test-Path $CoverageFile)) {
    Write-Error "Coverage file not found. Run the combat coverage collect command first."
    exit 1
}
...
# Combat visualizations + XP only (GiveCredits is economy, not combat text).
$scoped = @($game.classes.class | Where-Object {
    $_.filename -like '*\Combat\CombatTextCommand.cs' -or
    $_.filename -like '*\Packets\Sector\DamagePacket.cs' -or
    $_.filename -like '*\Packets\Sector\GiveXPPacket.cs'
...
Write-Output "Combat scoped line coverage: $rate%"
...
if ($rate -lt $MinimumRate -or $belowMinimum.Count -gt 0) {
    Write-Error "Combat scoped coverage $rate% failed gate (minimum $MinimumRate% overall and per file)."
    exit 1
}

Write-Output "Combat scoped coverage gate passed ($rate% >= $MinimumRate%)."
exit 0
