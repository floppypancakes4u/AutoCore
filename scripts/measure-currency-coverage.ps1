...
if ([string]::IsNullOrWhiteSpace($CoverageFile) -or -not (Test-Path $CoverageFile)) {
    Write-Error "Coverage file not found. Run the currency coverage collect command first."
    exit 1
}
...
# Currency economy + client sync (absolute CharacterLevel + GiveCredits delta packets).
$scoped = @($game.classes.class | Where-Object {
    $_.filename -like '*\Inventory\CurrencySync.cs' -or
    $_.filename -like '*\Inventory\AddCreditsResult.cs' -or
    $_.filename -like '*\Packets\Sector\GiveCreditsPacket.cs' -or
...
Write-Output "Currency scoped line coverage: $rate%"
...
if ($rate -lt $MinimumRate -or $belowMinimum.Count -gt 0) {
    Write-Error "Currency scoped coverage $rate% failed gate (minimum $MinimumRate% overall and per file)."
    exit 1
}

Write-Output "Currency scoped coverage gate passed ($rate% >= $MinimumRate%)."
exit 0
